...
grep -F "version: '0.1.0'" "$root/meson.build" >/dev/null || fail 'Meson project version is not 0.1.0'
...
grep -F 'PROJECT_NUMBER          = 0.1.0' "$root/Doxyfile" >/dev/null || fail 'Doxygen version is stale'
grep -F '## 0.1.0' "$root/HISTORY.md" >/dev/null || fail '0.1.0 history entry is missing'
